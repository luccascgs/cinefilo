...
function copyToClipboard(streak = 0) {
    ...
    navigator.clipboard.writeText(`Joguei cinefi.lol de ação ${emoji1.innerText} | 🔥${streak}`);
}
...
async function selectNext(order) {
    ...
    const streak = localStorage.getItem('actionStreak');
...
function storeSrike() {
    localStorage.setItem('actionStreak', 0);
    return 0;
}
...
function validToast(order, isValid) {
    ...
    const streakStoreage = localStorage.getItem('actionStreak');
    const streak = streakStoreage ? streakStoreage : storeSrike();
    localStorage.setItem('actionStreak', (Number(streak) + 1));
