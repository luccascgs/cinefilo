...
function copyToClipboard(streak = 0) {
    ...
    navigator.clipboard.writeText(`Joguei cinefi.lol geral ${emoji1.innerText} | 🔥${streak}`);
}
...
async function selectNext(order) {
    ...
    const streak = localStorage.getItem('geralStreak');
...
function validToast(order, isValid) {
    ...
    const streakStoreage = localStorage.getItem('geralStreak');
    const streak = streakStoreage ? streakStoreage : storeSrike();
    localStorage.setItem('geralStreak', (Number(streak) + 1));
