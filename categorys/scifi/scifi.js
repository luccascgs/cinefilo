...
function copyToClipboard(streak = 0) {
    ...
    navigator.clipboard.writeText(`Joguei cinefi.lol de ficção ${emoji1.innerText} | 🔥${streak}`);
}
...
async function selectNext(order) {
    ...
    const streak = localStorage.getItem('scifiStreak');
    ...
    if (isAcceptableName) {
        ...
        if (isMovieCorrect) {
            ...
            ///VOCÊ PERDEU
            if (order === 5) {
                localStorage.setItem('scifiStreak', 0);
...
function validToast(order, isValid) {
    ...
    const streakStoreage = localStorage.getItem('scifiStreak');
    const streak = streakStoreage ? streakStoreage : storeSrike();
    localStorage.setItem('scifiStreak', (Number(streak) + 1));
