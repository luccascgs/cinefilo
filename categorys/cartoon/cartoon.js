...
function copyToClipboard(streak = 0) {
    ...
    navigator.clipboard.writeText(`Joguei cinefi.lol de animação ${emoji1.innerText} | 🔥${streak}`);
}
...
function selectNext(order) {
    ...
    const streak = localStorage.getItem('cartoonStreak');
...
function validToast(order, isValid) {
    ...
    const streakStoreage = localStorage.getItem('cartoonStreak');
    const streak = streakStoreage ? streakStoreage : storeSrike();
    localStorage.setItem('cartoonStreak', (Number(streak) + 1));
