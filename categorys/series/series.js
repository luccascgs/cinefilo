...
function copyToClipboard(streak = 0) {
    ...
    navigator.clipboard.writeText(`Joguei cinefi.lol de séries & seriado ${emoji1.innerText} | 🔥${streak}`);
}
...
async function selectNext(order) {
    ...
    const streak = localStorage.getItem('seriesStreak');
    ...
    if (isAcceptableName) {
        ...
        if (isMovieCorrect) {
            ...
            ///VOCÊ PERDEU
            if (order === 5) {
                localStorage.setItem('seriesStreak', 0);
...
function validToast(order, isValid) {
    ...
    const streakStoreage = localStorage.getItem('seriesStreak');
    const streak = streakStoreage ? streakStoreage : storeSrike();
    localStorage.setItem('seriesStreak', (Number(streak) + 1));
