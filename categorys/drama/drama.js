...
function copyToClipboard(streak = 0) {
    ...
    navigator.clipboard.writeText(`Joguei cinefi.lol de drama ${emoji1.innerText} | 🔥${streak}`);
}
...
async function selectNext(order) {
    ...
    const streak = localStorage.getItem('dramaStreak');
...
function storeSrike() {
    localStorage.setItem('dramaStreak', 0);
    return 0;
}
...
function validToast(order, isValid) {
    ...
    const streakStoreage = localStorage.getItem('dramaStreak');
    const streak = streakStoreage ? streakStoreage : storeSrike();
    localStorage.setItem('dramaStreak', (Number(streak) + 1));
